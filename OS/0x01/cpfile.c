...
void osCpFile(int fdSrc, int fdDst){
    char buf[BUFF_SZ];
    ssize_t bytesRead;
    ...
    while((bytesRead = read(fdSrc, buf, sizeof buf)) > 0){
        ...
        //STDERR_FILENO 1
        //STDOUT_FILENO 2
        osAssert(-1 != write(fdDst, buf, bytesRead), "Write failed");
    }
...
int main(int argc, char **argv){
    ...
    int fdSrc, fdDst; //file descriptor
    osAssert(osOpenFile(argv[1],"r", &fdSrc), "Opening src failed");
    osAssert(osOpenFile(argv[2],"w", &fdDst), "Opening dst failed");

    osCpFile(fdSrc,fdDst);
    close(fdSrc);
    close(fdDst);
    return 0;
}
